...
import machine_coding_tictactoe.strategies.check_for_win.OrderOneWinningStrategy;
import machine_coding_tictactoe.strategies.check_for_win.PlayerWonStrategy;
...
public class Game {
        ...
        if(player instanceof HumanPlayer){
            ...
            if(humanPlayer.getPendingUndoCount() > 0){
                ...
                if(input.charAt(0) == 'y' || input.charAt(0) == 'Y'){
                   Move move = moves.remove(moves.size() - 1); // remove the last element from the moves array
                   Cell cell = move.getCell();
                   cell.removePlayer(); // remove the player from the cell
                   this.currentPlayerIdx = prevPlayerIdx; // give move access to the same player
                   winningStrategy.handleUndo(move); // update the map
            ...
            //do nothing, it's a bot, bot can not undo
        }
    }
    ...
    public static class GameBuilder{
        ...
        private int currentPlayerIdx;
        private List<Move> moves;
        private PlayerWonStrategy winingStrategy;
        ...
        public Game build() throws BotCountExceededException{
            ...
            for(Player p:players){
                ...
                if(botCount>1){
                    throw new BotCountExceededException("Found more than 1 bot, maximum only 1 bot is allowed");
                }
            }
